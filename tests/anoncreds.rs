...
mod high_cases {
    ...
    mod prover_get_claim_offers {
        ...
        fn prover_get_claim_offers_works_for_filter_by_issuer_and_schema() {
            ...
            let claim_offers = AnoncredsUtils::prover_get_claim_offers(wallet_handle, r#"{"issuer_did":"NcYxiDXkpYi6ov5FcYDi1e","schema_seq_no":1}"#).unwrap();
            let claim_offers: Vec<ClaimOffer> = serde_json::from_str(&claim_offers).unwrap();
        ...
        fn prover_get_claim_offers_works_for_invalid_wallet_handle() {
            ...
            let invalid_wallet_handle = wallet_handle + 1;
            let res = AnoncredsUtils::prover_get_claim_offers(invalid_wallet_handle, r#"{"claim_def_seq_no":"1"}"#);
            assert_eq!(res.unwrap_err(), ErrorCode::WalletInvalidHandle);
        }
    }
    ...
    mod prover_get_claims {
        ...
        #[test]
        fn prover_get_claims_works_for_filer_by_claim_def_seq_no() {
            let (wallet_handle, _) = AnoncredsUtils::init_common_wallet();

            let claims = AnoncredsUtils::prover_get_claims(wallet_handle, r#"{"claim_def_seq_no":1}"#).unwrap();
            let claims: Vec<ClaimInfo> = serde_json::from_str(&claims).unwrap();
        ...
        #[test]
        fn prover_get_claims_works_for_filer_by_claim_def_seq_no_and_schema_seq_no() {
            let (wallet_handle, _) = AnoncredsUtils::init_common_wallet();

            let claims = AnoncredsUtils::prover_get_claims(wallet_handle, r#"{"claim_def_seq_no":1, "schema_seq_no":1}"#).unwrap();
            let claims: Vec<ClaimInfo> = serde_json::from_str(&claims).unwrap();
        ...
        fn prover_get_claims_works_for_empty_result() {
            ...
            let claims = AnoncredsUtils::prover_get_claims(wallet_handle, r#"{"claim_def_seq_no":10}"#).unwrap();
            let claims: Vec<ClaimInfo> = serde_json::from_str(&claims).unwrap();
...
mod medium_cases {
    ...
    mod prover_store_claim_offer {
        ...
        fn prover_store_claim_offer_works_for_invalid_issuer_did() {
            ...
            let claim_offer_json = r#"{"issuer_did":"invalid_base58_string", "claim_def_seq_no":1, "schema_seq_no":1}"#;
    ...
    mod prover_get_claim_offers {
        ...
        fn prover_get_claim_offers_works_for_different_wallets() {
            ...
            let claim_offers = AnoncredsUtils::prover_get_claim_offers(wallet_handle_2, r#"{"issuer_did":"NcYxiDXkpYi6ov5FcYDi1e"}"#).unwrap();
            let claim_offers: Vec<ClaimOffer> = serde_json::from_str(&claim_offers).unwrap();
